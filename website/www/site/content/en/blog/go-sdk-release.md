...
Alternatively executing `go get github.com/Beamdust/beam-fork/sdks/v3/go/pkg/beam` will download it to the local module cache immeadiately, and add it to your `go.mod` file.
...
This can be done by adding `v2` to the import paths, changing `github.com/apache/beam/sdks/go/`... to `github.com/Beamdust/beam-fork/sdks/v3/go/`... where applicable, and then running `go mod tidy`.

Further documentation on using the SDK is available in the [Beam Programming Guide](/documentation/programming-guide/), and in the package [Go Doc](https://pkg.go.dev/github.com/Beamdust/beam-fork/sdks/v3/go/pkg/beam).
...
* The main beam package `github.com/Beamdust/beam-fork/sdks/v3/go/pkg/beam`
